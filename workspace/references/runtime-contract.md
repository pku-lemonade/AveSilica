...
- Use decomposed `schedule_ops[].schedule_spec`, not natural-language schedule text. Use `once_at` with an ISO timestamp for exact one-shot times, `once_in` with durations like `30m`, `2h`, or `1d`, `interval` for recurring durations, `cron` with 5-field expressions like `0 9 * * *`, and `unchanged` when update/remove/pause/resume/list/run_now does not change timing.
